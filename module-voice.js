...
const GREETINGS = {
  fr: "Bonjour, je suis Ode, avez-vous une question à me poser ?",
  nl: "Goedag, ik ben Ode, heeft u een vraag voor mij?",
  en: "Hello, I'm Ode, do you have a question for me?",
  de: "Guten Tag, ich bin Ode, haben Sie eine Frage an mich?",
  es: "Hola, soy Ode, ¿tiene alguna pregunta para mí?",
  ar: "مرحباً، أنا Ode، هل لديك سؤال تودّ طرحه؟",
};
...
/* ── Pré-chauffage du moteur TTS ──
   Sur la plupart des navigateurs (surtout Chrome), la 1ère synthèse vocale
   après le chargement de la page subit un temps de démarrage notable :
   - les voix "Google ..." (les meilleures, cf. PREFERRED_VOICES) sont
     synthétisées côté serveur : le navigateur doit établir une connexion
     réseau à ce service la toute première fois qu'elles sont utilisées ;
   - tant que cette connexion n'a pas eu lieu, chaque nouvel appel à cette
     voix précise repart de zéro.
   IMPORTANT : il faut donc pré-chauffer exactement LA MÊME voix que celle
   utilisée pour l'accueil (cf. _pickVoice/PREFERRED_VOICES), pas la voix
   par défaut du système — sinon le pré-chauffage ne sert à rien et le
   vrai délai de connexion se produit au moment de l'accueil.
   On lance ce pré-chauffage dès que ce module est chargé (et non au moment
   où l'overlay vocal s'ouvre), afin que tout soit déjà prêt quand l'accueil
   doit être prononcé. */
(function _warmUpTTS() {
  ...
  function _warmNow() {
    const l     = _lang();
    const voice = _pickVoice(l); /* met aussi en cache la voix pour plus tard */
    try {
      const warm = new SpeechSynthesisUtterance('.');
      warm.lang   = STT_LANG[l] || 'fr-BE';
      if (voice) warm.voice = voice; /* même voix que l'accueil réel */
      warm.volume = 0.01;            /* quasi muet mais pas 0 : certains
                                         navigateurs court-circuitent la
                                         synthèse réseau si volume=0 */
      window.speechSynthesis.speak(warm);
    } catch (e) {}
  }

  _ensureVoices(_warmNow);
})();
