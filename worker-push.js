...
/* ── Tokens admin temporaires (mémoire Worker, durée 1h) ── */
...
export default {
  async fetch(request, env) {
    ...
    /* ── POST /auth — valider le mot de passe admin et retourner un token ── */
    if (path === '/auth' && request.method === 'POST') {
      try {
        ...
        _adminTokens.set(token, Date.now() + 60 * 60 * 1000); /* 1h */
        return json({ token });
      } catch (e) {
    ...
    if (path === '/notify' && request.method === 'POST') {
      /* Vérification token admin (jamais le mot de passe en clair) */
